...
### PreToolUse(Skill) hook — スキルコンテキスト注入（汎用機能）

`scripts/skill_context.rb` は Skill ツール呼び出し前にスキル名で長期記憶を FTS5 検索し、上位5件を `## long-term-memory: <スキル名> (N件)` 形式で Claude のコンテキストに注入する汎用 hook。

**特徴:**
- **読み取り専用**（記憶は書き込まん）
- `NullEmbedder` で ONNX ロードをスキップ — FTS5 だけ使うので軽量・高速
- スキル名をそのままクエリに使うので、スキル名プリフィクス（例: `long-term-memory-*`）で記憶を紐付けておけば自動で引ける

`~/.claude/settings.local.json`（グローバル）に登録:
...
> **note:** 以前は Stop hook によるセッション自動保存 / PostToolUse hook によるツール操作キャプチャも備えていたが、ノイズが多く「長く覚えたいものを厳選する」方針に合わないため削除した。セッション保存が必要なら [claude-session-saver-mcp](https://github.com/bash0C7/claude-session-saver-mcp) を使う。
...
| `/long-term-memory-register` | 新 Mac セットアップ — gems・hooks・MCP 一括登録 |
| `/long-term-memory-hooks` | hook 設定確認・テスト・修正 |
...
│   ├── skill_context.rb        # PreToolUse(Skill) hook — スキル呼び出し前コンテキスト注入（汎用）
│   ├── capture_session.rb      # （旧）Stop hook — 現在は claude-session-saver-mcp 側で使用
│   ├── capture_tool_use.rb     # （旧）PostToolUse hook — 現在は未使用
