...
- Claude Code Stop / PostToolUse hook でセッション・ツール操作を自動キャプチャ
...
Stop/PostToolUse/PreToolUse hook を `~/.claude/settings.local.json` へマージし、Claude Desktop MCP も登録する。

---

## Claude Code hook 設定

全 hook は `~/.claude/settings.local.json`（グローバル）に設定する。

### Stop hook（セッションキャプチャ）

セッション終了時にトランスクリプトを自動保存。`role == "user"` / `"assistant"` のみ保存（system ロールは除外）。
...
### PostToolUse hook（ツール操作キャプチャ）

Edit / Write / Bash(curl/wget のみ) / WebFetch / WebSearch の結果を保存。コンテンツ長 200 文字未満はスキップ。
...
Skill ツール呼び出し前にスキル名で記憶を検索し、関連記憶をコンテキストとして注入する。
...
| スキル | 用途 |
|---|---|
| `/long-term-memory-register` | 新 Mac セットアップ — hooks・MCP 一括登録 |
| `/long-term-memory-setup` | gems インストール・テスト・MCP 登録案内 |
| `/long-term-memory-hooks` | hook 登録・確認・テスト |
...
| `/long-term-memory-maintenance` | 全操作リファレンス |
...
│   ├── capture_session.rb      # Stop hook — セッション自動キャプチャ
│   ├── capture_tool_use.rb     # PostToolUse hook — ツール操作キャプチャ
│   ├── skill_context.rb        # PreToolUse hook — Skill 呼び出し前コンテキスト注入
