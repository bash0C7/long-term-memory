...
| `lib/embedder.rb` | informers gem (ONNX) でテキスト→float[384] 変換 |
...
| 埋め込み生成 | informers gem (ONNX) | Python 不要・ローカル・all-MiniLM-L6-v2 |
...
- Stop hook で受け取れる会話内容の詳細（Claude Code の仕様を実装時に確認）
- all-MiniLM-L6-v2 は英語中心のモデル。日本語コンテキストが多い場合、`cl-nagoya/ruri-v3` などへの切り替えを検討（informers 対応次第）
